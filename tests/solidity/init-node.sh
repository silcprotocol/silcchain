...
CHAINDIR="$HOME/.tmp-silcd-solidity-tests" # TODO: make configurable like chain id
...
# Set client config
silcd config set client chain-id "$CHAINID" --home "$CHAINDIR"
silcd config set client keyring-backend "$KEYRING" --home "$CHAINDIR"
...
echo "$VAL_MNEMONIC" | silcd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER1_MNEMONIC" | silcd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER2_MNEMONIC" | silcd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER3_MNEMONIC" | silcd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER4_MNEMONIC" | silcd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"

# Set moniker and chain-id for Cosmos EVM (Moniker can be anything, chain-id must be an integer)
silcd init "$MONIKER" --chain-id "$CHAINID" --home "$CHAINDIR"
...
silcd genesis add-genesis-account "$(silcd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 100000000000000000000000000atest --keyring-backend "$KEYRING" --home "$CHAINDIR"
silcd genesis add-genesis-account "$(silcd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000atest --keyring-backend "$KEYRING" --home "$CHAINDIR"
silcd genesis add-genesis-account "$(silcd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000atest --keyring-backend "$KEYRING" --home "$CHAINDIR"
silcd genesis add-genesis-account "$(silcd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000atest --keyring-backend "$KEYRING" --home "$CHAINDIR"
silcd genesis add-genesis-account "$(silcd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000atest --keyring-backend "$KEYRING" --home "$CHAINDIR"
...
silcd genesis gentx "$VAL_KEY" 1000000000000000000000atest --gas-prices ${BASEFEE}atest --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
## In case you want to create multiple validators at genesis
## 1. Back to `silcd keys add` step, init more keys
## 2. Back to `silcd add-genesis-account` step, add balance for those
## 3. Clone this ~/.silcd home directory into some others, let's say `~/.clonedosd`
## 4. Run `gentx` in each of those folders
## 5. Copy the `gentx-*` folders under `~/.clonedosd/config/gentx/` folders into the original `~/.silcd/config/gentx`
...
# Collect genesis tx
silcd genesis collect-gentxs --home "$CHAINDIR"

# Run this to ensure everything worked and that the genesis file is setup correctly
silcd genesis validate-genesis --home "$CHAINDIR"
