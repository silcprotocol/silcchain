...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state["bank"]["denom_metadata"]=[{"description":"The native staking token for evmd.","denom_units":[{"denom":"sillet","exponent":0,"aliases":["attotest"]},{"denom":"test","exponent":18,"aliases":[]}],"base":"sillet","display":"test","name":"Test Token","symbol":"TEST","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	evmd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000sillet --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER1_KEY" 1000000000000000000000sillet --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER2_KEY" 1000000000000000000000sillet --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER3_KEY" 1000000000000000000000sillet --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER4_KEY" 1000000000000000000000sillet --keyring-backend "$KEYRING" --home "$HOMEDIR"

	# Sign genesis transaction
	evmd genesis gentx "$VAL_KEY" 1000000000000000000000sillet --gas-prices ${BASEFEE}sillet --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
