...
CHAINID="${CHAIN_ID:-silc_262154-1}"
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state["staking"]["params"]["bond_denom"]="asilc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="asilc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="asilc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["evm"]["params"]["evm_denom"]="asilc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["mint"]["params"]["mint_denom"]="asilc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

	# Add default token metadata to genesis
	jq '.app_state["bank"]["denom_metadata"]=[{"description":"The native staking token for evmd.","denom_units":[{"denom":"asilc","exponent":0,"aliases":["attotest"]},{"denom":"test","exponent":18,"aliases":[]}],"base":"asilc","display":"test","name":"Test Token","symbol":"TEST","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"asilc",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	evmd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000asilc --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER1_KEY" 1000000000000000000000asilc --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER2_KEY" 1000000000000000000000asilc --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER3_KEY" 1000000000000000000000asilc --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER4_KEY" 1000000000000000000000asilc --keyring-backend "$KEYRING" --home "$HOMEDIR"

	# Sign genesis transaction
	evmd genesis gentx "$VAL_KEY" 1000000000000000000000asilc --gas-prices ${BASEFEE}asilc --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
...
evmd start "$TRACE" \
	--log_level $LOGLEVEL \
	--minimum-gas-prices=0.0001asilc \
	--home "$HOMEDIR" \
